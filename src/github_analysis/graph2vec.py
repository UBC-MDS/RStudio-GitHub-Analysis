...
class Graph2Vec:
    ...
    def extract_features(self, projectGraphs):
        document_collections = Parallel(n_jobs=self.workers)(delayed(self.feature_extractor)(
            projectGraphs[g], self.iter, str(g)) for g in range(len(projectGraphs)))
    ...
    def get_embeddings(self, n_graphs):
        ...
        out = pd.DataFrame(out, columns=["x_" + str(dimension)
                                         for dimension in range(self.size)])
    ...
    def save_embeddings(self, n_graphs, output_path='./results/embeddings.csv',projectGraphsIndex=None):
        ...
        embeddings_ordered = embeddings[embeddings.columns.tolist(
        )[-1:] + embeddings.columns.tolist()[:-1]]
...
class WeisfeilerLehmanMachine:
    ...
    def do_a_recursion(self):
        ...
        for node in self.nodes:
            ...
            features = "_".join([str(self.features[node])] +
                                list(set(sorted([str(deg) for deg in degs]))))
