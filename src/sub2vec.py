...
def generateDegreeWalk(G, walkSize):
    """
    ...
    Output: a list, record the random walk generated, noted by the degree of each node
    """
...
def getDegreeLabelledGraph(G, rangetoLabels):
    ...
    Output: networkx graph, relabelled with degree
    """
...
def generateWalkFile(inputFile, walkFileName):
    """
    ...
    Output: a walk file, recorded all the walks
    """
    ...
    for pickle in pickles:
        ...
        degreeWalk = generateDegreeWalk(degreeGraph, walkSize)
...
def structural_embedding(inputFile, outputFile, dimensions=128, iterations=20, window=2):
    indexToName = generateWalkFile(inputFile, args.walkLength)
    sentences = doc.TaggedLineDocument(inputFile+'.walk')
    model = doc.Doc2Vec(sentences, size = dimensions, iter = iterations, window = window )
